...
my $otp_url = sprintf "otpauth://totp/%s?secret=%s&digits=%u&issuer=%s", uri_escape($conf->val('otpauth label').":$uname\@$realm"), uri_escape($secret_b32), $conf->val('otpauth digits'), uri_escape($conf->val('otpauth issuer'));
...
unless ($dry_run) {
	...
	if ($backend->update_user_secret(new ETVPN::Secret::RSA($conf), 'otpauth', $uname, $realm, $secret)) {
